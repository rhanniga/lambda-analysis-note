...
\end{subfigure}
\caption{N-sigma for protons (left) and pions (right) in the TOF detector as a function of $p_{T}$. If there is no TOF signal for the track, the TOF PID cut is not applied.}
\label{nsigma_tof}
\end{figure}

The $n\sigma$ values for the proton and pion are shown in Figure \ref{nsigma_proton} and Figure \ref{nsigma_pion}, respectively. 
...
\begin{itemize}
	\item \makebox[7.5cm][l]{unlike-sign $p\pi$ in $\Lambda$ mass region:}  \SI{1.014}{GeV/c^2}$< M_{KK} < $\SI{1.026}{GeV/c^2}
