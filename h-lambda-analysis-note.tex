...
The central points in this analysis are from $\Lambda$ candidates reconstructed using the V0 finder technique. The resonance technique serves as a powerful cross-check to ensure no topological biases are introduced within the V0 finder algorithm.


For both techniques, the daughter protons and pions were required to meet the following PID cuts using both the TPC and TOF detectors:
...
There were no additional topological cuts applied to the V0s or their corresponding daughters. This was done to maximize statistics and to minimize any topological biases introduced by reconstructing $\Lambda$s using the V0-finder algorithm itself. The invariant mass distribution in the 0-20\% multiplicity bin for the $\Lambda$s reconstructed using the V0 finder algorithm are shown in Figure \ref{v0_mass}. The $\Lambda$ peak is clearly visible with almost zero background. 

\begin{figure}[ht]
\centering
\includegraphics[width=3in]{figures/lambda_mass_0_20_v0_with_fit_zoomed.pdf}
\caption{Invariant mass distribution for $\Lambda$s reconstructed using the V0 finder algorithm in the 0-20\% multiplicity bin with $2 < \it{p}_T < 4$. The background from misidentified $\Lambda$s is negligible, despite no topological cuts being applied to the V0s.}
\label{v0_mass}
\end{figure}


 
 \subsection{Combinatorial Background Estimation (Resonance Technique Only)}
...
\end{figure}
 ...
 Measuring the $h-\Lambda$ correlation requires a clearly defined signal region for the unlike-sign $p\pi$ pairs, as well as an RSB region (in the case of the resonance technique). The values for each of the signal regions and RSB regions are as follows:

 \textbf{V0 Finder Technique:}
\begin{itemize}
	\item \makebox[7.5cm][l]{unlike-sign $p\pi$ in $\Lambda$ mass region:}  \SI{1.100}{GeV/c^2}$< M_{p\pi} < $\SI{1.134}{GeV/c^2}
\end{itemize}

 \textbf{Resonance Technique:}
\begin{itemize}
...
However, what is actually measured is an angular correlation of $h-(p\pi)$ pairs that is comprised of the real $h-\Lambda$ signal as well as a background of $h-(p\pi)$ pairs. For $\Lambda$s reconstructed using the V0 finder technique, this background is neglibile (see Section ). For the resonance technique, we must account for and subtract the combinatorial background contribution to the correlation. The full correlation is then given by:
...
Here, $r_{\text{Signal}}$ accounts for the signal that is missed by choosing a finite signal region, and is calculated using the extracted signal shown on the right side of Figure \ref{res_invariant_mass_0_20} by dividing the total number of $p\pi$ pairs in the signal region by the total number of $p\pi$ pairs across the entire distribution, then inverting. The fraction of signal missed is approximately X \% for each multiplicity bin. $r_{LS}$ is used to scale the unlike-sign $p\pi$ distribution in the RSB to match the combinatorial background in the signal region, and is calculated by first scaling the like-sign $p\pi$ distribution to the unlike-sign $p\pi$ distribution in the RSB, then using the scaled like-sign $p\pi$ to calculate the background integral in the signal region. Further details of this procedure can be found in Secion \ref{removecomb}.
