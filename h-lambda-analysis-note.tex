...
Due to the longer decay length of the $\Lambda$ ($c\tau \approx 10$ cm), the daughter proton and pion tracks will normally have less hits in each detector when comapared to the tracks of the trigger and associated hadrons, which are intended to be primary. Because of this, the tracks of the $\Lambda$ daughters are sometimes lost due to them being ``merged'' over by the trigger tracks during the Kalman filtering process (which favors the track with the most hits\ref{kalman}), provided the two tracks are close together in physical space (i.e. $\Delta\varphi, \Delta\eta \approx 0$). This problem is exacerbated by the fact that the $\Lambda$ decay is assymetric ($m_{p}/m_{\pi} \approx 7$), so the $\Lambda$ and daughter proton end up with similar momenta (and thus $\varphi$ and $\eta$). This means that whenever a proton from a $\Lambda$ decay is ``merged'' over by a trigger track, an $h-\Lambda$ pair with small $\Delta\varphi, \Delta\eta$ is lost. A simple diagram of this process is shown in Figure \ref{trackmerge_diagram}.
